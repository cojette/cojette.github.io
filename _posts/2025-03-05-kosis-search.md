...
소프트웨어 개발 속도가 가속화될수록, 데이터 아키텍처나 모델링 같은 데이터의 본질은 많은 경우 뒤로 밀려난다. 그러다보니 여전히 간단한 '딸깍' 만으로는 놓치는 게 너무 많다. 다들 편하게 나중에 AI로 정리하면 되지-라고 하지만 모든 개발에서 그랬듯 그 나중이란 없다. 속도가 빨라질 수록 더욱 그렇다. 하지만 데이터의 형태를 이해하고 내용의 맥락을 짚어낼 수 있다면, LLM은 더욱 강력한 가속기가 될 것이고, 데이터 쪽에도 더 많은 것을 기여할 수 있을 것이다. 그렇게 되길 기다린다.
